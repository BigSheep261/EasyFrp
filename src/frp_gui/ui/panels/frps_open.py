...
class FrpsOpenPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # 进程服务管理实际的 frps 生命周期，面板只调用其公开接口。
        self.frps_process_service = FrpsProcessService(parent=self)
        # 此标记区分用户点击与程序同步，防止 setChecked 递归触发业务操作。
    ...
    def _handle_frps_state_changed(self, state: str) -> None:
        ...
        # 运行中：选中并启用开关，使下一次点击能够请求停止。
        if state == "running":
            self._set_switch_checked(True)
            self.open_switch.setEnabled(True)
        # 已停止：取消选中并启用开关，使下一次点击能够请求启动。
        elif state == "stopped":
            self._set_switch_checked(False)
            self.open_switch.setEnabled(True)
