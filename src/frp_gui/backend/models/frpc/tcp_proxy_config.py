...
class TcpProxyConfig:
    ...
    name: str
    # local_port 指向本机等待被转发的 TCP 服务端口。
    local_port: int
    # remote_port 是 frps 对外监听并转发流量的端口。
    remote_port: int
    # local_ip 默认指向本机回环地址，减少服务意外暴露的范围。
    local_ip: str = "127.0.0.1"
    # TCP 模型的类型固定默认为 tcp，仍保留字段以支持反序列化校验。
    type: FrpcProxyType = FrpcProxyType.TCP

    def validate(self) -> None:
        """校验代理类型、名称、本地地址以及两端端口。"""
        # 防止误把其他代理类型的数据包装为 TCP 配置。
    ...
    def to_dict(self) -> dict[str, Any]:
        """校验并转换为结构化档案使用的 TCP 代理字典。"""
        # 序列化前确保所有字段均满足 TCP 代理约束。
        self.validate()
